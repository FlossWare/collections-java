...
public class LongList implements AutoCloseable {
    ...
    public LongList(File path) throws IOException {
    ...
    private void remapBuffer() throws IOException {
        ...
        long minSize = FileHeader.getHeaderSize() + (long) size.get() * LONG_SIZE;
        long fileSize = Math.max(file.length(), minSize + 1024 * LONG_SIZE);
        ...
        if (fileSize > MAX_MAPPED_SIZE) {
            throw new IOException("File too large to memory-map: " + fileSize +
                ...
                "Maximum LongList size is " + (MAX_MAPPED_SIZE / LONG_SIZE) + " longs.");
        }
    ...
    public long get(int index) {
        ...
        lock.readLock().lock();
        try {
            long position = FileHeader.getHeaderSize() + (long) index * LONG_SIZE;
            if (position > Integer.MAX_VALUE) {
                throw new IllegalStateException("Position overflow: list too large");
            }
            return mappedBuffer.getLong((int) position);
        } finally {
    ...
    public void add(long value) {
        ...
        try {
            ...
            long requiredSize = position + LONG_SIZE;
            if (requiredSize > file.length()) {
                long newSize = Math.max(requiredSize, file.length() + 1024 * LONG_SIZE);
                file.setLength(newSize);
                remapBuffer();
            }

            mappedBuffer.putLong((int) position, value);
    ...
    public void flush() {
        ...
        try {
            ...
            long actualSize = FileHeader.getHeaderSize() + (long) size.get() * LONG_SIZE;
            try {
                if (channel.isOpen()) {
